...
inline
signed mins(signed a, signed b) {
    return a < b ? a : b;
}

inline
unsigned minu(unsigned a, unsigned b) {
    return a < b ? a : b;
}

inline
signed long minsl(signed long a, signed long b) {
    return a < b ? a : b;
}
...
inline
signed long long minsll(signed long long a, signed long long b) {
    return a < b ? a : b;
}

inline
unsigned long long minull(unsigned long long a,
                          unsigned long long b) {
    return a < b ? a : b;
}

inline
void* minp(void* a, void* b) {
    return a < b ? a : b;
}
...
/**
 ** @brief Type-generic minimum for floating-point, wide integer and pointer values
 **/
#define min_alyr(A, B)                               \
_Generic(0 + (A) + (B) + 0,                     \
         signed: mins,                       \
...
/**
 ** @brief The minimum promoted value for @a XT, where XT
 ** can be an expression or a type name
 **
 ...
 **
 ** Comments:
 **  1) What's a portable value for UINT_MIN? In limits.h, there are #defines for INT_MAX and INT_MIN (and SHRT_* and LONG_* and so on), but only UINT_MAX.
 **   Should I define UINT_MIN myself? Is 0 (positive zero) a portable value? If you want to be "typesafe" you could use 0U, so if you use it in an expression
 **    you will have the correct promotions to unsigned
 **    Source: https://stackoverflow.com/questions/3499325/whats-a-portable-value-for-uint-min
 **/

#define minof_alyr(XT)                                \
_Generic(0+(XT)+0,                              \
         signed: INT_MIN,                       \
         unsigned: UINT_MIN,                    \
...
inline
unsigned long long maxull(unsigned long long a,
                          unsigned long long b) {
    return a < b ? b : a;
}

inline
unsigned long long maxusll(unsigned long long a,
                           signed long long b) {
    return b < 0 ? a : maxull(a, b);
}

inline
unsigned long long maxsull(signed long long a,
                           unsigned long long b) {
