...
char const* descend(char const* act,
                    ...
                    jmp_buf jmpTarget) {
    if (dp + 3 > sizeof head) longjmp(jmpTarget, tooDeep);
    ++dp;/*@\label{lab:incr-rec}*/
NEW_LINE:                             // Loops on output
    while (!act || !act[0]) {            // Loops for input
        ...
        if (!act) {                        // End of stream
            if (dp != 1) longjmp(jmpTarget, plusL);
            else goto ASCEND;
        }
    }
    ...
    for (; act && act[0]; ++act) { // Remainder of the line
        ...
        case RIGHT:                  // Returns on right brace
            if (dp == 1) longjmp(jmpTarget, plusR);
            else goto ASCEND;
...
void basic_blocks(void) {
    char buffer[maxline];
    unsigned depth = 0;
    unsigned volatile vdepth = depth;
    char const* format =
    ...
    fflush(stdout);
    fprintf(stderr, format, vdepth, LEFT, RIGHT);
    if (interrupt) {
